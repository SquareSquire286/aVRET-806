...
public class Movement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (OVRInput.GetDown(OVRInput.RawButton.RThumbstickLeft))
            camera.transform.rotation = Quaternion.Euler(camera.transform.rotation.eulerAngles.x, camera.transform.rotation.eulerAngles.y - 30.0f, camera.transform.rotation.eulerAngles.z);
